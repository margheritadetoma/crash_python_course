...
# Make the dir bandpass_margherita and copy all the files contained in bandpass_raw
...
# For each different extension 'e', it finds the files with
...
for e in ${extensions[@]}; do
	for file in *.$e; do
		...
		if [[ $new_string =~ [[:alpha:]] ]]; then  # check to control if the string that we got contains alphabet
			...
			mv -- "$file" "${file%.$e}._.filt"  # if it does not contain an alphabet letters, it puts just an underscore
		fi
		done
	done
